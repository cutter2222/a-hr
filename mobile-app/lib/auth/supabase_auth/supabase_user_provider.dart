...
class AlabugaAppSupabaseUser extends BaseAuthUser {
  AlabugaAppSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> alabugaAppSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AlabugaAppSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
