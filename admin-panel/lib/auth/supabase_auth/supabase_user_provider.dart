...
class AlabugaAdminSupabaseUser extends BaseAuthUser {
  AlabugaAdminSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> alabugaAdminSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = AlabugaAdminSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
